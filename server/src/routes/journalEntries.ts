...
router.get("/recent", authorise, async (req: Request & { user?: { id: string } }, res: Response) => {
  try {
    if (!req.user || !req.user.id) {
      return res.status(401).json({ message: "Unauthorised" });
    }
    ...
    const recentEntry = await pool.query(
      "SELECT * FROM journalentries WHERE user_id = $1 ORDER BY journalentry_created_at DESC LIMIT 1",
      [req.user.id]
    );
